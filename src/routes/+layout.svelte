<script lang="ts">
  ...
  const mount = async () => {
    ...
    if (
      ...
      window.location.host.includes("myrelay.page") ||
...
>
  <div class="flex-grow">
    <div class="flex flex-col items-center justify-center">
      <div
        id="mrp-wrapper"
        class="mrp-wrapper-bg mrp-wrapper-content transition-colors duration-200 ease-in-out"
      >
        <Header></Header>
        ...
        {#if !$MRP?.nostr?.relay?.config?.isComplete}
          <span
            class="mt-5 text-center block text-black/20 dark:text-white/20 italic text-lg"
          >
